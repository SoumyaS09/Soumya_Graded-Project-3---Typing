...
let character_text = document.querySelector(".init_cpm");
...
function ChangeContent() {
  ...
  content[ranIndex].split("").forEach(char => {
    let span = `<span>${char}</span>`
    content_text.innerHTML += span;
});
  content_text.querySelectorAll("span")[0].classList.add("active");
  document.addEventListener("keydown", () => inpField.focus());
  content_text.addEventListener("click", () => inpField.focus());
}
...
function finishGame() {
  ...
  cpm= typed - errors;
  character_text.textContent = cpm;
